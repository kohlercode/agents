...
final readonly class DeepSeekProvider implements LlmProviderInterface
{
    ...
    public function complete(LlmProviderRequest $request, array $providerConfig): LlmProviderResponse
    {
        ...
        if ($apiKey === '') {
            return new LlmProviderResponse('[DeepSeek API key missing]');
        }

        $baseUrl = (string)($providerConfig['api_base_url'] ?? 'https://api.deepseek.com');
        $payload = [
